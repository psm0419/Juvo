...
public class JuyusoServiceImpl implements JuyusoService {
	...
	public boolean fetchAndSaveJuyusoData(double lat, double lng) {
		try {
			...
			for (JsonNode stationNode : stations) {
				...
				Double hOilPrice = stationNode.path("PRICE").isMissingNode() ? null
						: stationNode.path("PRICE").asDouble();
	...
	private boolean isJuyusoDetailComplete(Juyuso juyuso) {
		return juyuso.getTel() != null && !juyuso.getTel().isEmpty() && juyuso.getVanAdr() != null
				&& !juyuso.getVanAdr().isEmpty() && juyuso.getGisXCoor() != null && juyuso.getGisYCoor() != null;
	}
	...
	private Juyuso fetchDetailData(String uniId) throws Exception {
		...
		if (!oilPrices.isMissingNode() && oilPrices.isArray()) {
			for (JsonNode priceNode : oilPrices) {
				...
				switch (prodcd) {
				case "B027":
					juyuso.setHOilPrice(price);
					break;
				case "B034":
					juyuso.setGOilPrice(price);
					break;
				case "D047":
					juyuso.setDOilPrice(price);
					break;
				case "C004":
					juyuso.setIOilPrice(price);
					break;
				case "K015":
					break;
				default:
					System.out.println("Unknown PRODCD: " + prodcd);
				}
			}
		}
		return juyuso;
	...
	public boolean updateJuyusoDetail(String uniId) {
		try {
			...
			System.out.println("Detail Juyuso Data - UNI_ID: " + newJuyuso.getUniId() + ", VAN_ADR: "
					+ newJuyuso.getVanAdr() + ", TEL: " + newJuyuso.getTel() + ", SIGUNCD: " + newJuyuso.getSiguncd()
					+ ", H_OIL_PRICE: " + newJuyuso.getHOilPrice() + ", G_OIL_PRICE: " + newJuyuso.getGOilPrice()
					+ ", D_OIL_PRICE: " + newJuyuso.getDOilPrice() + ", I_OIL_PRICE: " + newJuyuso.getIOilPrice());
	...
	public Map<String, Object> getReviewsByStationId(String uniId) {
		...
		double avgRating = reviews.stream().mapToDouble(r -> {
	...
	public boolean saveReview(String userId, String uniId, String content, double starCnt) {
		System.out.println("Saving review - userId: " + userId + ", uniId: " + uniId + ", starCnt: " + starCnt
				+ ", content: " + content);
	...
	// 즐겨찬기한 주유소 목록
	@Override
	public List<String> getFavoritesJuyuso(String userId) {
